...
import com.example.config.inventory.refresh.ConfigRefreshMetrics;
import com.example.config.inventory.refresh.ConfigSnapshotStatus;
import com.example.config.inventory.refresh.ConfigurationSnapshotProvider;
import com.example.config.inventory.refresh.ConfigurationValidationException;
import com.example.config.inventory.refresh.EnvironmentChangeKeyRecorder;
...
/**
 * Publishes validated, immutable {@link InventorySettings} snapshots to business code, and owns the
 * validate &rarr; compare &rarr; atomically swap &rarr; audit refresh algorithm that produces them.
 *
 * <p>This service owns its refresh plumbing outright. Nothing here is shared with the pricing
...
public class InventorySettingsProvider
  ...
  private static final Logger log = LoggerFactory.getLogger(InventorySettingsProvider.class);
  private static final String APPLICATION_NAME = "inventory-service";

  private final AtomicReference<InventorySettings> snapshot = new AtomicReference<>();
  ...
  public void applyRefresh(String trigger) {
    ...
    InventorySettings candidate = buildSnapshot();
  ...
  private InventorySettings buildSnapshot() {
