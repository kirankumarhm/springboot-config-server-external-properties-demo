...
class GlobalExceptionHandlerTest {
  ...
  @Test
  @DisplayName("handles OrderQuantityExceededException")
  void handlesOrderQuantityExceeded() {
    OrderQuantityExceededException ex = new OrderQuantityExceededException(600, 500);
    ProblemDetail problem = this.handler.onQuantityExceeded(ex);

    assertThat(problem.getStatus()).isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY.value());
    assertThat(problem.getTitle()).isEqualTo("Order quantity exceeded");
    assertThat(problem.getProperties()).containsEntry("requested", 600);
    assertThat(problem.getProperties()).containsEntry("allowed", 500);
  }
  ...
  void handlesMethodArgumentNotValid() {
    ...
    bindingResult.addError(new FieldError("target", "quantity", "must be greater than 0"));
