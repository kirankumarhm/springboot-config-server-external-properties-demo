#!/usr/bin/env bash
# Deploys version C (S3) to a local minikube cluster and verifies it end to end.
#
...
#   Set AWS_ENDPOINT empty and attach an IAM role (IRSA) for real AWS. For a local emulator you must also add a hostAliases entry mapping <bucket>.<endpoint-host> to the emulator Service ClusterIP, because the S3 client cannot be forced to path-style.
