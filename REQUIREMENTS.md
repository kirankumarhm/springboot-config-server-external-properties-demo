...
| ~~`AC-08`~~ **withdrawn** | ~~A secret stored in Git as a `{cipher}` value~~ | ~~A client fetches its configuration~~ | ~~The client receives the decrypted value; `grep` of the repository finds no plaintext secret; `/actuator/env` shows it masked~~ — **withdrawn:** the only `{cipher}` value (`inventory.downstream-api-key`) was removed, because an undecryptable ciphertext is served back as `invalid.<key>` and then aborts client startup on `@NotBlank`. `FR-04` and `NFR-10` are still asserted via `/encrypt` authorisation; `NFR-11` now holds trivially, as no configuration store contains a secret. | ~~`FR-04`, `NFR-11`, `NFR-12`~~ |
